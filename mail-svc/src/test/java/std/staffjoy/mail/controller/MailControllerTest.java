...
@EnableFeignClients(basePackages = {"std.staffjoy.mail.client"})
@Slf4j
public class MailControllerTest {
  ...
  public void testSendMail() throws ClientException {
    ...
    when(iAcsClient.getAcsResponse(any(SingleSendMailRequest.class)))
        .thenReturn(singleSendMailResponse);
    ...
    // verify email
    ArgumentCaptor<SingleSendMailRequest> argument = ArgumentCaptor
        .forClass(SingleSendMailRequest.class);
    ...
    // aliyun fail
    when(iAcsClient.getAcsResponse(any(SingleSendMailRequest.class)))
        .thenThrow(new ClientException("aliyun fail"));
