...
import std.staffjoy.whoami.TestConfig;
import std.staffjoy.whoami.client.WhoAmIClient;
...
@EnableFeignClients(basePackages = {"std.staffjoy.whoami.client"})
@Slf4j
public class WhoAmIControllerTest {
  ...
  public void testFindWhoAmI() {
    ...
    when(companyClient
        .getWorkerOf(AuthConstant.AUTHORIZATION_WHOAMI_SERVICE, TestConfig.TEST_USER_ID))
    ...
    when(companyClient
        .getAdminOf(AuthConstant.AUTHORIZATION_WHOAMI_SERVICE, TestConfig.TEST_USER_ID)).
        thenReturn(new GetAdminOfResponse(adminOfList));

    FindWhoAmIResponse findWhoAmIResponse = whoAmIClient
        .findWhoAmI(AuthConstant.AUTHORIZATION_AUTHENTICATED_USER);
  ...
  public void testGetIntercomSettings() throws Exception {
    ...
    when(accountClient
        .getAccount(AuthConstant.AUTHORIZATION_WHOAMI_SERVICE, TestConfig.TEST_USER_ID))
