package std.staffjoy.account.service.helper;
...
public class ServiceHelperTest {
  ...
  public void testSyncUserAsync() {
    ...
    when(companyClient.getWorkerOf(anyString(), eq(account.getId())))
    ...
    CompanyDto companyDto = CompanyDto.builder()
        ...
        .name("test_company")
        .build();
    when(companyClient.getCompany(anyString(), eq(companyId)))
    ...
    when(companyClient.getAdminOf(anyString(), eq(account.getId())))
    ...
    assertThat(user.getCustomAttributes().get("v2")).isEqualTo(
        CustomAttribute.newBooleanAttribute("v2", true));
    assertThat(user.getCustomAttributes().get("phonenumber"))
        .isEqualTo(CustomAttribute.newStringAttribute("phonenumber", account.getPhoneNumber()));
    assertThat(user.getCustomAttributes().get("confirmed_and_active")).isEqualTo(CustomAttribute
        .newBooleanAttribute("confirmed_and_active", account.isConfirmedAndActive()));
    ...
    assertThat(user.getCustomAttributes().get("is_staffjoy_support"))
        .isEqualTo(CustomAttribute.newBooleanAttribute("is_staffjoy_support", account.isSupport()));
