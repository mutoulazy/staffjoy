...
public class AccountService {
  ...
  public AccountDto create(String name, String email, String phoneNumber) {
    ...
    if (StringUtils.hasText(email)) {
      ...
      this.sendEmail(account.getId(), email, emailName, subject,
          AccountConstant.ACTIVATE_ACCOUNT_TMPL, true);
    }
  ...
  public AccountList list(int offset, int limit) {
    ...
    List<AccountDto> accountDtoList = accountPage.getContent().stream()
        .map(account -> convertToDto(account)).collect(toList());
  ...
  public AccountDto update(AccountDto newAccountDto) {
    ...
    if (!serviceHelper
        .isAlmostSameInstant(newAccount.getMemberSince(), existingAccount.getMemberSince())) {
      throw new ServiceException(ResultCode.REQ_REJECT, "You cannot modify the member_since date");
    }

    if (StringUtils.hasText(newAccount.getEmail()) && !newAccount.getEmail()
        .equals(existingAccount.getEmail())) {
    ...
    if (StringUtils.hasText(newAccount.getPhoneNumber()) && !newAccount.getPhoneNumber()
        .equals(existingAccount.getPhoneNumber())) {
  ...
  public void requestEmailChange(String userId, String email) {
    ...
    this.sendEmail(account.getId(), email, account.getName(), subject,
        AccountConstant.CONFIRM_EMAIL_TMPL, true);
  }
  ...
  void sendEmail(String userId, String email, String name, String subject, String template,
