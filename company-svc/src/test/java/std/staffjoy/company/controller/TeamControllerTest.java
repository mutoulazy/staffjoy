...
import std.staffjoy.company.dto.team.TeamList;
...
import std.staffjoy.company.model.Worker;
...
import std.staffjoy.company.repo.WorkerRepo;
...
public class TeamControllerTest {
  ...
  public void testTeam() {
    ...
    // list teams
    ListTeamResponse listTeamResponse = companyClient
        .listTeams(AuthConstant.AUTHORIZATION_AUTHENTICATED_USER, companyId);
    ...
    genericTeamResponse = companyClient
        .updateTeam(AuthConstant.AUTHORIZATION_AUTHENTICATED_USER, teamToUpdate);
    log.info(genericTeamResponse.toString());
    assertThat(genericTeamResponse.isSuccess()).isTrue();
    ...
    // get team
    genericTeamResponse = companyClient
        .getTeam(AuthConstant.AUTHORIZATION_AUTHENTICATED_USER, companyId, team2.getId());
    log.info(genericTeamResponse.toString());
    assertThat(genericTeamResponse.isSuccess()).isTrue();
    ...
    // getWorkerTeamInfo
    Worker worker = Worker.builder().userId(TestConfig.TEST_USER_ID).teamId(team2Found.getId())
        .build();
    ...
    GenericWorkerResponse genericWorkerResponse =
        companyClient.getWorkerTeamInfo(AuthConstant.AUTHORIZATION_AUTHENTICATED_USER, null,
