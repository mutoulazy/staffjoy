...
public class AdminControllerTest {
  ...
  public void testListThenDeleteAdmin() {
    ...
    // list admins
    when(companyService.getCompany(companyId1))
        .thenReturn(CompanyDto.builder().id(companyId1).build());
    when(companyService.getCompany(companyId2))
        .thenReturn(CompanyDto.builder().id(companyId2).build());

    ListAdminResponse listAdminResponse = companyClient
        .listAdmins(AuthConstant.AUTHORIZATION_SUPPORT_USER, companyId1);
    // verify
    ...
    listAdminResponse = companyClient
        .listAdmins(AuthConstant.AUTHORIZATION_SUPPORT_USER, companyId2);
    // verify
    log.info(listAdminResponse.toString());
    assertThat(listAdminResponse.isSuccess()).isTrue();
    ...
    // test adminOf
    GetAdminOfResponse getAdminOfResponse = companyClient
        .getAdminOf(AuthConstant.AUTHORIZATION_ACCOUNT_SERVICE, userId);
    // verify
    ...
    List<String> companyIds = getAdminOfResponse.getAdminOfList().getCompanies().stream()
        .map(o -> o.getId())
        .collect(Collectors.toList());
    assertThat(companyIds).containsExactly(companyId1, companyId2);

    // delete admin2
    BaseResponse baseResponse = companyClient
        .deleteAdmin(AuthConstant.AUTHORIZATION_SUPPORT_USER, directoryEntryRequest2);
  ...
  public void testCreateThenGetAdmin() {
    ...
    genericDirectoryResponse = companyClient
        .getAdmin(AuthConstant.AUTHORIZATION_WWW_SERVICE, companyId, userId);
    // verify
    assertThat(genericDirectoryResponse.isSuccess()).isTrue();
