...
public class CompanyControllerTest {
  ...
  public void testUpdateCompany() {
    when(accountClient.trackEvent(any(TrackEventRequest.class))).thenReturn(
        BaseResponse.builder().message("company created").build());
    ...
    CompanyDto companyDto = CompanyDto.builder()
        ...
        .build();
    GenericCompanyResponse genericCompanyResponse = companyClient
        .createCompany(AuthConstant.AUTHORIZATION_SUPPORT_USER, companyDto);
    ...
    genericCompanyResponse = companyClient
        .updateCompany(AuthConstant.AUTHORIZATION_SUPPORT_USER, companyDto);
    ...
    assertThat(genericCompanyResponse.getResultCode()).isEqualTo(ResultCode.NOT_FOUND);
  }

  @Test
  public void testListCompany() {
    when(accountClient.trackEvent(any(TrackEventRequest.class)))
        .thenReturn(BaseResponse.builder().message("company created").build());
    ...
    CompanyDto companyDto = CompanyDto.builder()
        ...
        .build();
    GenericCompanyResponse genericCompanyResponse = companyClient
        .createCompany(AuthConstant.AUTHORIZATION_WWW_SERVICE, companyDto);
    assertThat(genericCompanyResponse.isSuccess()).isTrue();

    // list company and verify
    ListCompanyResponse listCompanyResponse = companyClient
        .listCompanies(AuthConstant.AUTHORIZATION_SUPPORT_USER, 0, 2);
    ...
    // list company and verify
    listCompanyResponse = companyClient
        .listCompanies(AuthConstant.AUTHORIZATION_SUPPORT_USER, 0, 2);
    log.info((listCompanyResponse.toString()));
    assertThat(listCompanyResponse.isSuccess()).isTrue();
    ...
    // list company and verify
    listCompanyResponse = companyClient
        .listCompanies(AuthConstant.AUTHORIZATION_SUPPORT_USER, 1, 2);
    log.info((listCompanyResponse.toString()));
    assertThat(listCompanyResponse.isSuccess()).isTrue();
    companyList = listCompanyResponse.getCompanyList();
    assertThat(companyList.getCompanies()).hasSize(1);
  ...
  public void testCreateThenGetCompany() {
    ...
    // test getCompany
    genericCompanyResponse = companyClient
        .getCompany(AuthConstant.AUTHORIZATION_ACCOUNT_SERVICE, createdCompany.getId());
    log.info(genericCompanyResponse.toString());
    assertThat(genericCompanyResponse.isSuccess()).isTrue();
