...
public class WorkerService {
  ...
  public WorkerEntries listWorkers(String companyId, String teamId) {
    ...
    WorkerEntries workerEntries = WorkerEntries.builder().companyId(companyId).teamId(teamId)
        .build();
    for (Worker worker : workerList) {
      DirectoryEntryDto directoryEntryDto = directoryService
          .getDirectoryEntry(companyId, worker.getUserId());
  ...
  public DirectoryEntryDto createWorker(WorkerDto workerDto) {
    ...
    DirectoryEntryDto directoryEntryDto = directoryService
        .getDirectoryEntry(workerDto.getCompanyId(), workerDto.getUserId());
    ...
    try {
      Worker workerToCreate = Worker.builder().teamId(workerDto.getTeamId())
          .userId(workerDto.getUserId()).build();
