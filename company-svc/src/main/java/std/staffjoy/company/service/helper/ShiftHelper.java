package std.staffjoy.company.service.helper;
...
import std.staffjoy.company.service.TeamService;
...
//    }
  ...
  public ShiftDto updateShift(ShiftDto shiftDtoToUpdate, boolean suppressNotification) {
    ...
    ShiftDto orig = this.getShift(shiftDtoToUpdate.getId(), shiftDtoToUpdate.getTeamId(),
    ...
    if (!StringUtils.isEmpty(shiftDtoToUpdate.getUserId())) {
      // validate and will throw exception if not exist
      directoryService
          .getDirectoryEntry(shiftDtoToUpdate.getCompanyId(), shiftDtoToUpdate.getUserId());
    }

    if (!StringUtils.isEmpty(shiftDtoToUpdate.getJobId())) {
      // validate and will throw exception if not exist
      jobService.getJob(shiftDtoToUpdate.getJobId(), shiftDtoToUpdate.getCompanyId(),
