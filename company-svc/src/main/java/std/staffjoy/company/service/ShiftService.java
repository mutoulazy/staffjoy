...
public class ShiftService {
  ...
  public ShiftList listWorkerShifts(WorkerShiftListRequest req) {
    ...
    List<Shift> shifts = shiftRepo
        .listWorkerShifts(req.getTeamId(), req.getWorkerId(), req.getShiftStartAfter(),
            req.getShiftStartBefore());
  ...
  public ShiftList listShifts(ShiftListRequest req) {
    ...
    if (!StringUtils.isEmpty(req.getUserId()) && StringUtils.isEmpty(req.getJobId())) {
      shifts = shiftRepo
          .listWorkerShifts(req.getTeamId(), req.getUserId(), req.getShiftStartAfter(),
              req.getShiftStartBefore());
    }

    if (!StringUtils.isEmpty(req.getJobId()) && StringUtils.isEmpty(req.getUserId())) {
      shifts = shiftRepo.listShiftByJobId(req.getTeamId(), req.getJobId(), req.getShiftStartAfter(),
          req.getShiftStartBefore());
    }

    if (!StringUtils.isEmpty(req.getJobId()) && !StringUtils.isEmpty(req.getUserId())) {
      shifts = shiftRepo.listShiftByUserIdAndJobId(req.getTeamId(), req.getUserId(), req.getJobId(),
          req.getShiftStartAfter(), req.getShiftStartBefore());
    }

    if (StringUtils.isEmpty(req.getJobId()) && StringUtils.isEmpty(req.getUserId())) {
      shifts = shiftRepo.listShiftByTeamIdOnly(req.getTeamId(), req.getShiftStartAfter(),
          req.getShiftStartBefore());
    }
  ...
  public void deleteShift(String shiftId, String teamId, String companyId) {
    ...
    if (!StringUtils.isEmpty(orig.getUserId()) && orig.isPublished() && orig.getStart()
        .isAfter(Instant.now())) {
