...
import std.staffjoy.company.dto.Association;
import std.staffjoy.company.dto.AssociationList;
...
import std.staffjoy.company.model.Directory;
...
public class DirectoryService {
  ...
  public DirectoryEntryDto createDirectory(NewDirectoryEntry req) {
    ...
    try {
      genericAccountResponse = accountClient
          .getOrCreateAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, getOrCreateRequest);
    } catch (Exception ex) {
    ...
    DirectoryEntryDto directoryEntryDto = DirectoryEntryDto.builder()
        .internalId(req.getInternalId()).companyId(req.getCompanyId()).build();
    copyAccountToDirectory(account, directoryEntryDto);

    boolean directoryExists =
        directoryRepo.findByCompanyIdAndUserId(req.getCompanyId(), account.getId()) != null;
    if (directoryExists) {
  ...
  public DirectoryList listDirectory(String companyId, int offset, int limit) {
    ...
    for (Directory directory : directoryPage.getContent()) {
      ...
      try {
        resp = accountClient
            .getAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, directoryEntryDto.getUserId());
      } catch (Exception ex) {
  ...
  public DirectoryEntryDto getDirectoryEntry(String companyId, String userId) {
    DirectoryEntryDto directoryEntryDto = DirectoryEntryDto.builder().userId(userId)
    ...
    if (directory == null) {
      throw new ServiceException(ResultCode.NOT_FOUND,
          "directory entry not found for user in this company");
    }
  ...
  public DirectoryEntryDto updateDirectoryEntry(DirectoryEntryDto request) {
    ...
    try {
      genericAccountResponse1 = accountClient
          .getAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, orig.getUserId());
    } catch (Exception ex) {
    ...
    if (account.isConfirmedAndActive() && accountUpdateRequested) {
      throw new ServiceException(ResultCode.PARAM_VALID_ERROR,
          "this user is active, so they cannot be modified");
    ...
    if (accountUpdateRequested) {
      ...
      try {
        genericAccountResponse2 = accountClient
            .updateAccount(AuthConstant.AUTHORIZATION_COMPANY_SERVICE, account);
      } catch (Exception ex) {
    ...
    try {
      directoryRepo
          .updateInternalIdByCompanyIdAndUserId(request.getInternalId(), request.getCompanyId(),
              request.getUserId());
    } catch (Exception ex) {
    ...
    if (!request.isConfirmedAndActive() &&
        (!orig.getPhoneNumber().equals(request.getPhoneNumber()) || (
            "".equals(request.getPhoneNumber()) && !orig.getEmail().equals(request.getEmail())))) {
      OnboardWorkerRequest onboardWorkerRequest = OnboardWorkerRequest.builder()
          .companyId(request.getCompanyId())
