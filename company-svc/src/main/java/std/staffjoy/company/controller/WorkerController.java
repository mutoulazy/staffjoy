...
import std.staffjoy.company.service.WorkerService;
...
public class WorkerController {
  ...
  public GenericDirectoryResponse getWorker(@RequestParam String companyId,
      @RequestParam String teamId, @RequestParam String userId) {
    if (AuthConstant.AUTHORIZATION_AUTHENTICATED_USER.equals(AuthContext.getAuthz())) {
      permissionService.checkPermissionTeamWorker(companyId, teamId);
    }
  ...
  public BaseResponse deleteWorker(@RequestBody @Validated WorkerDto workerDto) {
    ...
    workerService
        .deleteWorker(workerDto.getCompanyId(), workerDto.getTeamId(), workerDto.getUserId());
